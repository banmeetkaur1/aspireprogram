...
#db25-> text splitter idea
#db26-> new splits idea with prefixing
persist_directory= 'db26'
#reading csvs
loader = TextLoader("programlist.txt", encoding = 'utf-8')
...
#testing out new split method
#splits = list(df["Structure"])
splits = df.apply(lambda row: f"{row['Program Name']}: {row['Structure']}", axis=1).tolist()
...
if os.path.exists(persist_directory):
    ...
    vectorstore = Chroma.from_texts(texts=splits, embedding=embeddings , persist_directory= persist_directory)
